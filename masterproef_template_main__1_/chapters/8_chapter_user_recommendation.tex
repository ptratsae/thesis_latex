...
Because the model is trained with the 2:1:1 negative sampling strategy (Section~\ref{sec:negative_sampling}), geographic preference is learned directly from the training signal: users are contrasted against jobs from different states during training, so the model internalises state-level location as a genuine compatibility feature. No post-processing boost is applied at inference — same-state jobs surface naturally at the top of the ranking as a consequence of what the model has learned. Recommendations are restricted to jobs from the active evaluation window to avoid surfacing positions that are no longer open.