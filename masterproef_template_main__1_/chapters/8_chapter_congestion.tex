...
recommendation at scale. Throughout this chapter and the congestion experiments (\ref{sec:exp_congestion}), each user is assumed to apply to exactly the top-$k_u$ recommended jobs, where $k_u$ matches their historical application count. Both forms are first
...
This chapter starts with the job-side market-maker intervention in Section~\ref{sec:job_congestion} and candidate-side market-maker intervention in Section~\ref{sec:user_congestion}. This is followed by the bilateral scoring approach in Section~\ref{sec:bilateral_scoring}, which combines the user and recruiter models into a single score. Finally, the complete two-phase pipeline is described in Section~\ref{sec:hybrid_strategy}, which integrates all three previous components into a unified system.  
% ============================================================
...
job. Each user applies to their top-$k_u$ jobs by $s_{uj}$, where $k_u$
...
surfaced. The top-$k_{\text{short}}$ candidates by $\hat{r}_{uj}$ are shortlisted for
each job. The experimental results of the complete two-phase pipeline are presented in Section~\ref{sec:exp_hybrid_results}.