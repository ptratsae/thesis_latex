...
The prompt is developed iteratively, with each revision guided by inspection of outputs on sampled pairs. The key insight driving the design is that without structured criteria, the \gls{LLM} applies inconsistent implicit thresholds across independent calls. Explicit \gls{CoT} instructions resolve this by forcing the model to reason through the same criteria for every pair before assigning a fit level and explicit rules for evaluation make sure the model is consistent. The output is returned in JSON format. The full prompt is detailed in Section~\ref{sec:exp_recruiter_datagen}.
